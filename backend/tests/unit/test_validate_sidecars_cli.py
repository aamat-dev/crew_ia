...
def test_validate_sidecars_cli(tmp_path: Path) -> None:
    ...
    # exécution normale
    res = subprocess.run(
        [sys.executable, "backend/tools/validate_sidecars.py"],
        env=env,
        capture_output=True,
        text=True,
    )
    ...
    res = subprocess.run(
        [sys.executable, "backend/tools/validate_sidecars.py", "--strict"],
        env=env,
        capture_output=True,
        text=True,
    )
    ...
    res = subprocess.run(
        [
            sys.executable,
            "backend/tools/validate_sidecars.py",
            "--since",
    ...
    res = subprocess.run(
        [sys.executable, "backend/tools/validate_sidecars.py", "--since", run1],
        env=env,
        capture_output=True,
        text=True,
    )
